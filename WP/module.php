  ...
  init(payload) {
        ...
        const applyHeatingCircuitTemperatureColors = (card) => {
            ...
            circuits.forEach((circuit) => {
                ...
                /*
                 * Fußbodenheizung: den bereits sichtbaren ORIGINALPFAD selbst
                 * animieren. Keine Kopie, kein <use>, kein Overlay.
                 *
                 * Genau dieser Pfad erhält hier auch den Temperaturverlauf,
                 * daher kann die Animation nicht mehr auf einem falschen oder
                 * unsichtbaren SVG-Element landen.
                 */
                const directFloor =
                    svg.querySelector(
                ...
                const directFloorActive = !!(
                    !hotWaterActive
                    && String(circuit.type || 'off') === 'underfloor'
                    && (
                        stateIsOn(
                            circuit.key === '1'
                ...
                if (directFloor) {
                    if (directFloorActive) {
                        directFloor.style.setProperty(
                            'stroke-dasharray',
                            '5 6',
                            'important'
                        );
                        ...
                        directFloor.style.removeProperty(
                            'stroke-dasharray'
                        );
                        directFloor.style.removeProperty(
                            'stroke-linecap'
                 ...
                 */
                // Fußbodenheizung wird oben direkt auf dem Originalpfad animiert.
        ...
        const applyFlowAnimations = (card) => {
            ...
            const animateEmitter = (
                ...
                /*
                 * Heizkörper: auch hier den bereits von uns erzeugten
                 * Serpentinenpfad DIREKT animieren, statt nochmals einen
                 * Overlay-Klon darüberzulegen.
                 */
                const radiatorFlowActive =
                    !!enabled && radiatorVisible;

                flowPath.style.setProperty(
                    'display',
                    radiatorFlowActive ? 'inline' : 'none',
                    'important'
                );
                flowPath.style.setProperty(
                    'visibility',
                    radiatorFlowActive ? 'visible' : 'hidden',
                    'important'
                );
                flowPath.style.setProperty(
                    'fill',
