...
(() => {
    ...
    const applyRefrigerantCircuitMode = (card) => {
        ...
        /*
         * Wärmetauscher-Grafik ebenfalls tauschen.
         *
         * Die Original-SVG besitzt je nach Card-Version keine stabilen IDs
         * für die eigentlichen Wärmetauscher-Symbole. Deshalb werden die
         * Symbolteile geometrisch anhand der bereits sicher vorhandenen
         * Text-/Messwertgruppen erkannt.
         *
         * Wichtig: Texte, Messwerte, Leitungen und große Hintergrundelemente
         * bleiben unangetastet. Nur kompakte grafische Elemente unmittelbar
         * um Verdampfer bzw. Kondensator wechseln horizontal die Seite.
         */
        const swapHeatExchangerGraphics = () => {
            if (svg.dataset.symconHeatExchangerGraphicsSwapped === '1') {
                return;
            }

            const evaporatorRefs = [
                svg.querySelector('#textEvaporator'),
                svg.querySelector('#textEvaporatorPressure'),
                svg.querySelector('#textEvaporatorTemperature')
            ].filter(Boolean);

            const condenserRefs = [
                svg.querySelector('#textCondenser'),
                svg.querySelector('#textCondenserPressure'),
                svg.querySelector('#textCondenserTemperature')
            ].filter(Boolean);

            if (evaporatorRefs.length === 0 || condenserRefs.length === 0) {
                return;
            }
            ...
            const evaporatorBox = unionBox(evaporatorRefs);
            const condenserBox = unionBox(condenserRefs);

            const deltaX = condenserBox.cx - evaporatorBox.cx;
            if (!Number.isFinite(deltaX) || Math.abs(deltaX) < 10) {
                return;
            }

            /*
             * Suchbereich bewusst großzügiger als die Textgruppe:
             * Die Wärmetauscherzeichnung sitzt direkt um ihre Beschriftung.
             * Große Leitungen, SVG-Hintergrund usw. werden über Größe und
             * Elementtyp ausgeschlossen.
             */
            const verticalMin = Math.min(evaporatorBox.y, condenserBox.y) - 110;
            const verticalMax = Math.max(
                evaporatorBox.y + evaporatorBox.height,
                condenserBox.y + condenserBox.height
            ) + 110;

            const horizontalRadius = Math.max(
                95,
                Math.max(evaporatorBox.width, condenserBox.width) / 2 + 60
            );
            ...
            const isProtected = (element) => {
                ...
                const tag = String(element.tagName || '').toLowerCase();
                return ['svg', 'defs', 'style', 'lineargradient', 'radialgradient', 'stop'].includes(tag);
            };

            const sideOf = (cx) => {
                if (Math.abs(cx - evaporatorBox.cx) <= horizontalRadius) {
                    return -1;
                }
                if (Math.abs(cx - condenserBox.cx) <= horizontalRadius) {
                    return 1;
                }
                return 0;
            };
            ...
            Array.from(svg.querySelectorAll('path, rect, circle, ellipse, polygon, polyline, use, g'))
                .forEach((element) => {
                    ...
                    if (cy < verticalMin || cy > verticalMax) {
                        return;
                    }
