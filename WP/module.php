...
(() => {
    ...
    const applyRefrigerantCircuitMode = (card) => {
        ...
        /*
         * Im Kühlbetrieb NUR die beiden kleinen Wärmetauscher-Symbole tauschen:
         *
         *   links  neben "Verdampfer"   = kleiner Kreis mit Wärmetauscher
         *   rechts neben "Verflüssiger" = kleiner Kreis mit Tropfen
         *
         * Heizwendel, Umwälzpumpe, Verdichter, Leitungen usw. werden
        ...
        const swapHeatExchangerSymbols = () => {
            ...
            const evaporatorAnchor = boxCenter(evaporatorText);
            const condenserAnchor = boxCenter(condenserText);

            /*
             * Beide gesuchten Symbole besitzen in der Original-Card einen
             * kleinen Kreis. Wir verwenden genau diesen Kreis als Anker.
             *
             * Der große Kältekreis wird über den Radius ausgeschlossen,
             * ebenso kleinere Deko-/Pumpenelemente über Richtung und Abstand.
             */
            const findSymbolCircle = (anchor, direction) => {
                const candidates = [];

                Array.from(svg.querySelectorAll('circle')).forEach((circle) => {
                    const radius = parseFloat(circle.getAttribute('r') || '0');

                    // Wärmetauscher-Symbolkreise sind klein/mittelgroß.
                    // Großen Kältekreis und winzige Punkte ausschließen.
                    if (!Number.isFinite(radius) || radius < 12 || radius > 35) {
                        return;
                    }

                    let center;
                    try {
                        center = boxCenter(circle);
                    } catch (error) {
                        return;
                    }
                    ...
                    // Verdampfer-Symbol liegt links vom Text,
                    // Verflüssiger-Symbol rechts vom Text.
                    if (direction < 0 && dx >= -10) {
                        return;
                    }
                    ...
                    // Nur Symbole im direkten Umfeld des Textes.
            ...
            const leftCircle = findSymbolCircle(evaporatorAnchor, -1);
            const rightCircle = findSymbolCircle(condenserAnchor, 1);

            if (!leftCircle || !rightCircle || leftCircle.circle === rightCircle.circle) {
                return;
            }

            /*
             * Zum Kreis die kleinste kompakte Gruppe suchen, die das gesamte
             * Symbol enthält, aber keinen Text und keine fremden Card-Elemente.
             */
            const findCompactSymbolGroup = (circle) => {
                let group = circle.parentElement;
                let best = circle;
                ...
                while (group && group !== svg) {
                    ...
                    if (group.querySelector('text, tspan')) {
                        break;
                    }
                    ...
                    // Das vollständige kleine Symbol passt sicher hier hinein.
                    // Heizwendel/Pumpe/Leitungen erzeugen deutlich größere Gruppen.
                    if (
                        box.width > 0
                        && box.height > 0
                        && box.width <= 75
            ...
            const leftSymbol = findCompactSymbolGroup(leftCircle.circle);
            const rightSymbol = findCompactSymbolGroup(rightCircle.circle);

            if (!leftSymbol || !rightSymbol || leftSymbol === rightSymbol) {
                return;
            }

            const leftCenter = boxCenter(leftSymbol);
            const rightCenter = boxCenter(rightSymbol);

            const moveSymbol = (element, dx, dy) => {
            ...
            // Exakt die beiden ursprünglichen Symbol-Mittelpunkte vertauschen.
            moveSymbol(
                leftSymbol,
